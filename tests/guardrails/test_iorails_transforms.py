...
def _mask_both_config() -> dict:
    """Input and output masking on the non-streaming generate path."""
    config = copy.deepcopy(_mask_input_config())
    config["rails"]["output"] = {"flows": ["mask sensitive data on output"]}
...
def _mask_both_streaming_config() -> dict:
    ...
    config = copy.deepcopy(_mask_both_config())
    config["rails"]["output"]["streaming"] = {
        "enabled": True,
        "chunk_size": 4,
        "context_size": 2,
...
@pytest.mark.asyncio
async def test_generate_output_rails_see_the_rewritten_user_turn():
    """After an input TRANSFORM, generate() output rails are handed the rewritten user turn."""
    seen: list[str] = []

    async with started_iorails(_mask_both_config()) as iorails:
        _stub_main_chat(iorails, MAIN_OUTPUT)
        _install_iorails_action(iorails, "mask sensitive data on input", RailDirection.INPUT, _redact_user)
        _install_iorails_action(iorails, "mask sensitive data on output", RailDirection.OUTPUT, _redact_bot)
        original = iorails.rails_manager.is_output_safe
        ...
        iorails.rails_manager.is_output_safe = _spy
        await iorails.generate_async(messages=[{"role": "user", "content": SECRET_INPUT}])
