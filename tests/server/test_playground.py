...
def test_playground_assets_and_prompt_bank():
    ...
    assert {item["category"] for item in bank} >= {"allowed", "jailbreak", "harmful", "mask", "output"}
...
def test_playground_chat_blocks_jailbreak_and_allows_safe_prompt():
    ...
    assert any(rail.get("name") == "regex check input" for rail in blocked["guardrails"]["log"]["activated_rails"])
...
def test_playground_masks_pii_instead_of_blocking():
    payload = _complete("My social security number is 123-45-6789. Can you confirm you received it?")
    content = payload["choices"][0]["message"]["content"]
    assert "redacted" in content.lower()
    assert "123-45-6789" not in content
    assert not payload["guardrails"]["output_data"].get("triggered_input_rail")
    assert any(rail.get("name") == "mask pii" for rail in payload["guardrails"]["log"]["activated_rails"])


def test_root_redirects_to_playground_when_chat_ui_is_unavailable():
