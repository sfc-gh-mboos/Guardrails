...
def _serialize_check(result: Any) -> dict[str, Any]:
    return {"status": result.status.value, "content": result.content, "rail": result.rail}
...
async def compare(req: CompareRequest) -> dict[str, Any]:
    ...
    if req.api == "check" and req.mode == "both":
        raise HTTPException(
            status_code=400,
            detail="check() with both surfaces needs a user turn and an assistant turn; use generate(), or pick input or output.",
        )
    ...
    if req.engine in {"llmrails", "both"}:
        ...
        if req.api == "check":
            ...
            results.append({"engine": "llmrails", "api": "generate", "response": generated})
    ...
    if req.engine in {"iorails", "both"}:
        ...
        async with iorails:
            ...
            _stub_main_chat(iorails, bot_stub)
            if req.api == "check":
                ...
                results.append({"engine": "iorails", "api": "generate", "response": generated})
