...
`IORails` supports those NeMoGuard flows plus catalog rails that rewrite the user or bot turn (see [Transforms](#transforms)).
...
| Feature | LLMRails | IORails | Notes |
|---|:---:|:---:|---|
| Input rewrite (`user_message`) | ✓ | ✓ | Applied before the main model call |
| Output rewrite (`bot_message`) | ✓ | ✓ | Applied to the returned assistant text |
| Retrieval rewrite (`relevant_chunks`) | ✓ | ✗ | IORails has no retrieval context |
| Streaming output rewrite | ✗ | ✗ | Streaming treats a transform as non-blocking and does not rewrite already-emitted chunks |

Both engines apply `TRANSFORM` outcomes from catalog rails that mask or otherwise rewrite the active user or bot turn.
On `LLMRails` the Colang flow writes `$user_message` or `$bot_message` from `RailOutcome.transform_text`.
On `IORails`, `RailsManager` applies the same rewrite to the conversation messages or the assistant text.

Retrieval-chunk transforms remain an `LLMRails` capability.
Streaming output rails do not apply a rewrite on either engine; they still block when the rail returns `BLOCK`.
...
| `check` / `check_async` (rails-only validation) | ✓ | ✓ | Both engines apply input/output transforms |
...
Both engines expose `generate`, `generate_async`, `stream_async`, `check`, and `check_async`.
...
| Feature | LLMRails | IORails | Notes |
|---|:---:|:---:|---|
| Community integrations (PII, AlignScore, ActiveFence, and others) | ✓ | ◐ | IORails runs input/output transform rails; retrieval and other catalog rails fall back |
...
`IORails` runs the catalog rails that rewrite the user or bot turn, such as PII masking, Prompt Security, AutoAlign, CrowdStrike AIDR, Pangea, and injection detection.
Retrieval-chunk transforms, fact-checking rails that need retrieved evidence, and catalog rails that only block without a transform still route to `LLMRails`.
