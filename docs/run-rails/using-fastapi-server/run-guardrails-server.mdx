...
The server uses [FastAPI](https://fastapi.tiangolo.com/) and includes a built-in playground for inspecting prompt input, output, and rail impact.
...
The server includes a built-in playground for testing guardrails configurations.
Open `http://localhost:8000/` after starting the server to:

- Choose a configuration and a prompt from the example prompt bank.
- Confirm the input that is sent to `/v1/chat/completions`.
- Inspect the model output and the rails that activated, including stop decisions.
- Optionally compare the same prompt with rails turned off.

If you installed the `chat-ui` extra, the conversation Chat UI remains available at `http://localhost:8000/chat`.
...
Use this endpoint rather than `GET /` for health probes, because `GET /` serves the playground when the UI is enabled.
