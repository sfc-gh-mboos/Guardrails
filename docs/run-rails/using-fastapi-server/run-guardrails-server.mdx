...
The server includes a simple prompt playground at `http://localhost:8000/playground`.
Use it to confirm the input you send, the guarded output, and which rails blocked or transformed the turn.

The playground ships with an example prompt bank covering allowed, jailbreak, harmful, PII/mask, output-rail, and off-topic cases.
The `examples/configs/playground` configuration uses catalog regex rails, a PII mask action, and a canned echo model, so you can exercise pass, block, and rewrite paths without an LLM API key:

```bash
nemoguardrails server --config examples/configs --default-config-id playground
```

Open `http://localhost:8000/playground`. You can deep-link an example with `?prompt=<id>`, for example `?prompt=jailbreak-ignore`.
The playground sends `guardrails.preserve_config_model: true` so `/v1/chat/completions` keeps the configuration's main model instead of replacing it with `MAIN_MODEL_ENGINE`.

When the Chainlit chat UI extra is not installed, `GET /` redirects to the playground.

## Chat UI

The server includes a built-in Chat UI for testing guardrails configurations.
Access it at `http://localhost:8000/` after starting the server when the `chat-ui` extra is installed.

<Info>

The Chat UI and prompt playground are for internal testing only.
...
Use this endpoint rather than `GET /` for health probes, because `GET /` redirects to the chat UI or prompt playground when those UIs are enabled.
