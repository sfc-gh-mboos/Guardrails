...
The inspector UI shows the original prompt, the message after input rails, the final reply, and which rails ran, modified, or blocked the turn.

Start the server with the offline demo config (keyword rails and a canned model, no API key):

```bash
nemoguardrails server --config examples/configs/inspector_demo
```

Open `http://localhost:8000/inspect`. The page loads an example prompt bank covering a safe question, a jailbreak, a harmful request, PII masking, and an output-rail secret leak.

You can also inspect a prompt over HTTP. This endpoint does not inject a request `model` into the config, so demo engines keep their configured LLM:
...
The response includes `input`, `output`, `user_message_after_rails`, `triggered_input_rail`, `triggered_output_rail`, and `activated_rails`.

`--disable-chat-ui` also hides `GET /inspect`. The `POST /v1/inspect` API remains available.
...
- [Inspect Rail Impact](api:POST/v1/inspect)
