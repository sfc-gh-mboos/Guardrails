...
- Includes a built-in prompt playground and chat UI for testing.
...
### View the Prompt Playground

Open `http://localhost:8000/playground` to confirm prompt input, guarded output, and which rails fired.
The `examples/configs/playground` configuration works without an LLM API key.

### View the Chat UI

Open `http://localhost:8000` in your browser to access the built-in chat UI for testing when the `chat-ui` extra is installed.
