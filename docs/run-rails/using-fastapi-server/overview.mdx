...
Open `http://localhost:8000/playground` to inspect prompt input, guarded output, and rail impact.
If the Chat UI extra is not installed, `http://localhost:8000` serves the same playground.

### View the Chat UI

When the `chat-ui` extra is installed, open `http://localhost:8000/chat` for the conversational Chat UI.
