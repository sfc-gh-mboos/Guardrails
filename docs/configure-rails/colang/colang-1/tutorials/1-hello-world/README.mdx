...
You can also test a guardrails configuration using the NeMo Guardrails server and the playground.
...
The playground is available at `http://localhost:8000`. Use it to confirm the prompt you send, the guarded output, and which rails activated.
If you installed the `chat-ui` extra, the conversation Chat UI is available at `http://localhost:8000/chat`:
