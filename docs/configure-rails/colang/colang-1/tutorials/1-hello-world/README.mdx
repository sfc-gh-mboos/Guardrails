...
The prompt playground is available at `http://localhost:8000/playground`.
The Chat UI interface is available at `http://localhost:8000` when the `chat-ui` extra is installed:
