...
This command starts the library server with the example configurations. The prompt playground is accessible at `http://localhost:8000/playground`. The Chat UI is accessible at `http://localhost:8000` when the `chat-ui` extra is installed.
